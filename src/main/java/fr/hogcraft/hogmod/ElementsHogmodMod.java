...
public class ElementsHogmodMod implements IFuelHandler, IWorldGenerator {
	...
	public void preInit(FMLPreInitializationEvent event) {
		try {
			for (ASMDataTable.ASMData asmData : event.getAsmData().getAll(ModElement.Tag.class.getName())) {
				...
				if (clazz.getSuperclass() == ElementsHogmodMod.ModElement.class)
					elements.add((ElementsHogmodMod.ModElement) clazz.getConstructor(this.getClass()).newInstance(this));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		Collections.sort(elements);
		elements.forEach(ElementsHogmodMod.ModElement::initElements);
		this.addNetworkMessage(HogmodModVariables.WorldSavedDataSyncMessageHandler.class, HogmodModVariables.WorldSavedDataSyncMessage.class,
	...
	public void onPlayerLoggedIn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote) {
			WorldSavedData mapdata = HogmodModVariables.MapVariables.get(event.player.world);
			WorldSavedData worlddata = HogmodModVariables.WorldVariables.get(event.player.world);
			if (mapdata != null)
				HogmodMod.PACKET_HANDLER.sendTo(new HogmodModVariables.WorldSavedDataSyncMessage(0, mapdata), (EntityPlayerMP) event.player);
			if (worlddata != null)
				HogmodMod.PACKET_HANDLER.sendTo(new HogmodModVariables.WorldSavedDataSyncMessage(1, worlddata), (EntityPlayerMP) event.player);
		}
	}
	...
	public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler, Class<T> messageClass,
			Side... sides) {
		for (Side side : sides)
			HogmodMod.PACKET_HANDLER.registerMessage(handler, messageClass, messageID, side);
		messageID++;
	}
	...
	public static class ModElement implements Comparable<ModElement> {
		...
		protected final ElementsHogmodMod elements;
		protected final int sortid;
		public ModElement(ElementsHogmodMod elements, int sortid) {
